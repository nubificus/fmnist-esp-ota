...
This script calculates the maximum equal size for 2 app partitions and
...
if [[ "$#" == 1 && "$1" == "--help" ]]; then
	...
	# Check if the first argument is a valid flash size
	input_size="$1"
	...
	>&2 echo "# Error: Invalid number of arguments. Use --help for usage information."
	exit 1
fi
...
if [[ -n "$tflite_file" ]]; then
	...
	avail=$(($tflite_off - $ota0_off))
else
	tflite_aligned_size=0
	tflite_off=0
	avail=$(($flash_size - $ota0_off))
fi

# TODO: Replace 0 with a bear minimum app size
...
max_image_size=$((avail / 2))

# Align the app image size to 64KB i.e. round down to the closest multiple of 64KB
aligned_image_size=$(( (max_image_size / align) * align ))

image1_off=$ota0_off
image2_off=$((ota0_off + aligned_image_size))

# Ensure the image offsets are 64KB aligned
for addr in $image1_off $image2_off; do
...
export TFLITE_MODEL_SIZE=$tflite_size
